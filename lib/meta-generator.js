...
function generateHeaderLinks(config) {
    ...
    config.header.links.forEach(function(v, k) {
        let selected = ""
        if (v.url === window.location.pathname) { selected = " selected"};
        links += "<a href='"+v.url+"'><div class='header-link-div"+selected+" menu-header-link-div'>"+v.name+"</div></a>";
    });
    $("#links").html(links);
}
    ...
    waitForElm('#pinned-posts').then((elm) => {
        ...
        for (const [key, postfileName] of Object.entries(file.pinnedPosts)) {
            ...
            postfileNameSplit.forEach(function(v, k) {
                console.log(v)
                pinnedPost = next["/"+v];
		if (next["/"+v] !== undefined && next["/"+v].children !== undefined) {
